...
const PAVIMENTOLAB_VERSION = "v21.4-indexeddb-popup-calibracao";
...
function showModal(title,text,loading=false){
  ...
  // Durante calibração/consolidação não mostra botões de exportação.
...
function computeRoughness(samples){
  ...
  const baseline = state.current?.calibration?.baseline_mean || 0;
  const vals = samples
    .map(v=>Math.abs(v - baseline))
  ...
  const index = m + (std * 0.5) + Math.min(1, peaks / 10);
...
function showCalibrationScreen(){
  if(!els.calibrationOverlay) return;
  els.calibrationOverlay.classList.remove("hidden");
  if(els.calibrationMessage) els.calibrationMessage.textContent = "Mantenha o celular parado na posição de coleta.";
  if(els.calibrationCount) els.calibrationCount.textContent = "10";
  if(els.calibrationProgress) els.calibrationProgress.style.width = "0%";
  if(els.calibrationSamples) els.calibrationSamples.textContent = "0 amostras coletadas";
}

function updateCalibrationScreen(elapsedMs, samplesCount){
  const total = CALIBRATION_MS || 10000;
  const remaining = Math.max(0, Math.ceil((total - elapsedMs) / 1000));
  const progress = Math.min(100, Math.round((elapsedMs / total) * 100));

  if(els.calibrationCount) els.calibrationCount.textContent = String(remaining);
  if(els.calibrationProgress) els.calibrationProgress.style.width = `${progress}%`;
  if(els.calibrationSamples) els.calibrationSamples.textContent = `${samplesCount} amostras coletadas`;
  if(els.calibrationMessage){
    els.calibrationMessage.textContent = remaining > 0
      ? "Não mova o celular. A gravação começa em seguida."
      : "Calibração concluída. Iniciando gravação...";
  }
}
...
async function calibrateSensors(){
  ...
  while(Date.now() - start < CALIBRATION_MS){
    if(Array.isArray(state.motionBuffer) && state.motionBuffer.length){
      samples.push(...state.motionBuffer.slice(-60));
    }

    const elapsedMs = Date.now() - start;
    updateCalibrationScreen(elapsedMs, samples.length);

    await new Promise(r=>setTimeout(r,200));
  }

  updateCalibrationScreen(CALIBRATION_MS, samples.length);

  const vals = samples.length ? samples : (state.motionBuffer || []).slice(-600);
  const m = vals.length ? mean(vals) : 0;
  const std = vals.length ? Math.sqrt(mean(vals.map(v=>(v-m)**2))) : 0;
...
boot().catch(err=>{
  console.error(err);
  alert("Erro ao iniciar PavimentoLab v21.4: " + (err.message || err));
});
