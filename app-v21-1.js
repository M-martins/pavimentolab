...
const PAVIMENTOLAB_VERSION = "v21.1-indexeddb-old-thresholds";
...
function showModal(title,text,loading=false){
  ...
  els.modalActions.classList.toggle("hidden",!!loading);
...
function computeRoughness(samples){
  ...
  // v21.1: escala antiga, sem reclassificação posterior.
  const vals = samples
...
async function calibrateSensors(){
  showModal("Calibrando sensores","Mantenha o celular na posição de coleta. Aguarde 10 segundos...",true);
  ...
  while(Date.now() - start < CALIBRATION_MS){
    if(state.motionBuffer.length){
      samples.push(...state.motionBuffer.slice(-30));
    }

    const elapsed = Math.min(CALIBRATION_MS, Date.now() - start);
    const remaining = Math.max(0, Math.ceil((CALIBRATION_MS - elapsed)/1000));
    const collected = samples.length;

    els.modalText.textContent =
      `Calibrando sensores... faltam ${remaining}s · ${collected} amostras`;
  ...
  // Zera o buffer depois da calibração para a coleta começar limpa.
  state.motionBuffer.splice(0);

  els.modalText.textContent = "Calibração concluída. Iniciando coleta...";
...
boot().catch(err=>{
  console.error(err);
  alert("Erro ao iniciar PavimentoLab v21.1: " + (err.message || err));
});
