const PAVIMENTOLAB_VERSION = "v14-metadata-zip";
...
const els = {
  ...
  historyList:document.getElementById("historyList"), btnClearAll:document.getElementById("btnClearAll"),
...
const STORAGE_KEY="pavimentolab_collections_v14";
const DRAFT_KEY="pavimentolab_current_v14";
...
async function stopCollection(){
  ...
  showModal("Consolidando dados","Aguarde. Estou salvando a corrida no histórico antes de liberar a exportação.",true);

  // Dá tempo para a tela do modal aparecer antes de processar/salvar.
  ...
  try{
    ...
    const p=collection.pauses.at(-1);
    if(p&&!p.end)p.end=new Date().toISOString();

    // Marca metadados de consolidação.
    collection.finalizedAt = new Date().toISOString();
    collection.version = PAVIMENTOLAB_VERSION;
    collection.summary = summarizeCollection(collection);

    // CRÍTICO: salva primeiro. Antes de mexer em UI, export ou modal.
    saveCollection(collection);
    localStorage.removeItem(DRAFT_KEY);
    state.lastStoppedCollection=collection;

    // Limpa estado da coleta depois de salvo.
    state.current=null;
    ...
    // O release pode falhar em alguns Androids, então não pode travar o encerramento.
    releaseWakeLock().catch(()=>{});

    // Dá mais um respiro para a UI atualizar.
    await new Promise(r=>setTimeout(r,180));

    showExportModal(collection);
  }catch(err){
    console.error("Erro ao finalizar coleta",err);
    showModal("Coleta salva parcialmente","Houve um erro ao finalizar a interface, mas tente verificar o histórico. Se aparecer lá, os dados foram preservados.",false);
  }finally{
...
function pointsGeo(c){return JSON.stringify({type:"FeatureCollection",name:`${c.name}_pontos_brutos`,metadata:{version:c.version,startedAt:c.startedAt,endedAt:c.endedAt,totalDistanceM:c.totalDistanceM,aggregation_m:AGGREGATION_M,anti_drift_min_step_m:ANTI_DRIFT_M,summary:c.summary||summarizeCollection(c),device:c.deviceInfo||getDeviceInfo(),calibration:c.calibration,visibilityEvents:c.visibilityEvents||[]},features:c.points.filter(p=>p.lat&&p.lon).map(p=>({type:"Feature",geometry:{type:"Point",coordinates:[p.lon,p.lat]},properties:{...p,collection:c.name}}))},null,2)}
function simplePointsGeo(c){const f=[];c.segments.forEach(s=>{f.push({type:"Feature",geometry:{type:"Point",coordinates:[s.lon_start,s.lat_start]},properties:{point_role:"inicio_trecho",segment_id:s.segment_id,timestamp:s.timestamp_start,gps_accuracy_m:s.gps_accuracy_start_m,distance_m:s.distance_m,roughness_index:s.roughness_index,classe:s.classe,segment_quality:s.segment_quality,collection:c.name}});f.push({type:"Feature",geometry:{type:"Point",coordinates:[s.lon_end,s.lat_end]},properties:{point_role:"fim_trecho",segment_id:s.segment_id,timestamp:s.timestamp_end,gps_accuracy_m:s.gps_accuracy_end_m,distance_m:s.distance_m,roughness_index:s.roughness_index,classe:s.classe,segment_quality:s.segment_quality,collection:c.name}})});return JSON.stringify({type:"FeatureCollection",name:`${c.name}_pontos_simplificados`,metadata:{version:c.version,aggregation_m:AGGREGATION_M,anti_drift_min_step_m:ANTI_DRIFT_M,simplification:"start_end_points_per_segment",summary:c.summary||summarizeCollection(c),device:c.deviceInfo||getDeviceInfo()},features:f},null,2)}
function segmentsGeo(c){return JSON.stringify({type:"FeatureCollection",name:`${c.name}_trechos`,metadata:{version:c.version,startedAt:c.startedAt,endedAt:c.endedAt,totalDistanceM:c.totalDistanceM,aggregation_m:AGGREGATION_M,anti_drift_min_step_m:ANTI_DRIFT_M,summary:c.summary||summarizeCollection(c),device:c.deviceInfo||getDeviceInfo(),calibration:c.calibration,visibilityEvents:c.visibilityEvents||[]},features:c.segments.map(s=>({type:"Feature",geometry:{type:"LineString",coordinates:[[s.lon_start,s.lat_start],[s.lon_end,s.lat_end]]},properties:{...s,collection:c.name}}))},null,2)}
...
function summaryJson(c){
  return JSON.stringify(summarizeCollection(c), null, 2);
}
...
function recoverDraft(){const raw=localStorage.getItem(DRAFT_KEY);if(!raw)return;try{const d=JSON.parse(raw);if(d&&!d.endedAt&&confirm("Encontrei uma coleta não finalizada. Salvar no histórico?")){d.endedAt=new Date().toISOString();d.recovered=true;saveCollection(d);localStorage.removeItem(DRAFT_KEY);renderHistory()}}catch(e){}}
...
els.btnMenu.onclick=openDrawer;els.btnCloseDrawer.onclick=closeDrawer;els.drawer.onclick=e=>{if(e.target===els.drawer)closeDrawer()};els.btnLocate.onclick=locate;els.btnPause.onclick=togglePause;els.btnCloseModal.onclick=hideModal;els.btnMain.addEventListener("pointerdown",startHold);els.btnMain.addEventListener("pointerup",cancelHold);els.btnMain.addEventListener("pointerleave",cancelHold);els.btnMain.addEventListener("pointercancel",cancelHold);els.btnClearAll.onclick=()=>{if(confirm("Apagar todo histórico?")){localStorage.removeItem(STORAGE_KEY);renderHistory()}};
