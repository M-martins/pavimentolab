...
const PAVIMENTOLAB_VERSION = "v21.2-indexeddb-calibracao-obrigatoria";
...
async function calibrateSensors(){
  ...
  showModal(
    "Calibrando sensores",
    "Mantenha o celular parado na posição de coleta. A gravação começa automaticamente ao final.",
    true
  );
  ...
  // Limpa amostras antigas antes da calibração.
  ...
  while(Date.now() - start < CALIBRATION_MS){
    ...
    const remaining = Math.max(0, Math.ceil((CALIBRATION_MS - elapsedMs) / 1000));
    const elapsed = Math.min(10, Math.floor(elapsedMs / 1000));

    if(els.modalText){
      els.modalText.textContent =
        `Calibrando sensores... ${elapsed}s / 10s · ${samples.length} amostras`;
    }
  ...
  // Limpa novamente para a coleta começar sem sujeira da calibração.
  if(Array.isArray(state.motionBuffer)) state.motionBuffer.splice(0);

  if(els.modalText){
    els.modalText.textContent = "Calibração concluída. Iniciando gravação...";
  }
...
boot().catch(err=>{
  console.error(err);
  alert("Erro ao iniciar PavimentoLab v21.2: " + (err.message || err));
});
