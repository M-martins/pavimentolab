const PAVIMENTOLAB_VERSION = "v18-data-recovery";
...
const STORAGE_KEY="pavimentolab_collections_v18";
const DRAFT_KEY="pavimentolab_current_v18";
const LEGACY_COLLECTION_KEYS=["pavimentolab_collections_v5","pavimentolab_collections_v12","pavimentolab_collections_v13","pavimentolab_collections_v14","pavimentolab_collections_v15","pavimentolab_collections_v16","pavimentolab_collections_v17"];
const LEGACY_DRAFT_KEYS=["pavimentolab_current_v5","pavimentolab_current_v12","pavimentolab_current_v13","pavimentolab_current_v14","pavimentolab_current_v15","pavimentolab_current_v16","pavimentolab_current_v17"];
...
function locate(){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    els.gpsPill.textContent="GPS indisponível";
  },{enableHighAccuracy:true,maximumAge:0,timeout:12000})
}

function initialLocateOnOpen(){
...
function updateDiagnostics(){const now=Date.now();const gpsAge=state.lastGpsAt?(now-state.lastGpsAt)/1000:null;const avg=state.gpsIntervals.length?mean(state.gpsIntervals)/1000:null;els.diagGps.textContent=state.lastGpsPosition?.accuracy?`${state.lastGpsPosition.accuracy.toFixed(0)}m`:"--";els.diagMotion.textContent=state.motionHz?`${state.motionHz.toFixed(0)} Hz`:"--";els.diagWake.textContent=state.wakeLock?"ativa":"não ativa";els.diagSave.textContent=state.lastSaveAt?`salvo ${state.saveCounter}`:"--";els.diagGpsAge.textContent=gpsAge==null?"--":gpsAge<1?"agora":`${gpsAge.toFixed(0)}s`;els.diagGpsInterval.textContent=avg?`${avg.toFixed(1)}s`:"--";els.diagMotionHz.textContent=state.motionHz?`${state.motionHz.toFixed(0)} Hz`:"--";els.diagVisibility.textContent=document.visibilityState==="visible"?"ativa":"2º plano";
  if(els.diagHistoryCount) els.diagHistoryCount.textContent = `${getCollections().length} coleta(s)`;
  if(els.diagDraftCount) els.diagDraftCount.textContent = `${countDrafts()} rascunho(s)`;
  if(els.diagStorageSize) els.diagStorageSize.textContent = bytesToHuman(localStorageSize());
  if(els.diagStorageKey) els.diagStorageKey.textContent = STORAGE_KEY.replace("pavimentolab_collections_","");
}
...
function openDrawer(){els.drawer.classList.add("open");renderHistory();updateDiagnostics();renderLocalScanResults()}function closeDrawer(){els.drawer.classList.remove("open")}
...
function looksLikeCollection(obj){
  return obj && typeof obj === "object" &&
    ...
    (obj.name || obj.startedAt || obj.id);
}

function normalizeRecoveredCollection(c, sourceKey){
  const copy = {...c};
  copy.id = copy.id || (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}_${Math.random()}`);
  copy.name = copy.name || `coleta_recuperada_${nowId()}`;
  copy.startedAt = copy.startedAt || copy.createdAt || new Date().toISOString();
  copy.endedAt = copy.endedAt || copy.finalizedAt || new Date().toISOString();
  copy.version = copy.version || "versao_antiga";
  copy.recovered = true;
  copy.recoveredFrom = sourceKey;
  copy.points = Array.isArray(copy.points) ? copy.points : [];
  copy.segments = Array.isArray(copy.segments) ? copy.segments : [];
  copy.totalDistanceM = copy.totalDistanceM || copy.segments.reduce((s,x)=>s+(x.distance_m||0),0);
  try{ copy.summary = safeSummarizeCollection(copy); }catch{}
  return copy;
}

function findCollectionsInValue(value, key){
  const found = [];
  try{
    ...
    if(Array.isArray(parsed)){
      parsed.forEach(item=>{
        if(looksLikeCollection(item)) found.push(normalizeRecoveredCollection(item, key));
      });
    }else if(looksLikeCollection(parsed)){
      found.push(normalizeRecoveredCollection(parsed, key));
...
function scanAllLocalStorage(){
  ...
  for(let i=0;i<localStorage.length;i++){
    ...
    const maybeRelevant =
      keyLower.includes("pavimentolab") ||
      keyLower.includes("coleta") ||
      keyLower.includes("collection") ||
      keyLower.includes("current") ||
      value.includes("roughness_index") ||
      value.includes("segments") ||
      value.includes("pontos");

    if(!maybeRelevant) continue;

    const collections = findCollectionsInValue(value, key);
...
function renderLocalScanResults(){
  const results = scanAllLocalStorage();
  els.localDataList.innerHTML = "";

  if(!results.length){
    els.localDataList.innerHTML = `<p class="local-note">Nenhum dado antigo encontrado neste domínio.</p>`;
    return;
  }
  ...
  results.forEach((res, idx)=>{
    ...
    const totalPoints = res.collections.reduce((s,c)=>s+(c.points?.length||0),0);
    const totalSegments = res.collections.reduce((s,c)=>s+(c.segments?.length||0),0);
    div.innerHTML = `
      <strong>${res.key}</strong>
      <small>${res.count} coleta(s) · ${totalPoints} pontos · ${totalSegments} trechos · ${(res.size/1024).toFixed(1)} KB</small>
      <div class="local-actions">
        <button data-import-local="${idx}">importar</button>
        <button data-backup-local="${idx}">backup</button>
      </div>
    `;
    els.localDataList.appendChild(div);
  });

  els.localDataList.querySelectorAll("[data-import-local]").forEach(btn=>{
    btn.onclick=()=>{
      const res = results[Number(btn.dataset.importLocal)];
      importLocalCollections(res.collections, res.key);
    };
  });

  els.localDataList.querySelectorAll("[data-backup-local]").forEach(btn=>{
    btn.onclick=()=>{
      const res = results[Number(btn.dataset.backupLocal)];
      download(
        `pavimentolab_local_${res.key}_${new Date().toISOString().replaceAll(":","-")}.json`,
        JSON.stringify({app_version:PAVIMENTOLAB_VERSION,source_key:res.key,exported_at:new Date().toISOString(),collections:res.collections},null,2),
        "application/json"
      );
    };
  });
...
function importLocalCollections(collections, sourceLabel="local_scan"){
  const existing = getCollections();
  const existingFingerprints = new Set(existing.map(collectionFingerprint));
  const merged = [...existing];
  let imported = 0;
  let skipped = 0;

  (collections || []).forEach(c=>{
    const normalized = normalizeRecoveredCollection(c, c.recoveredFrom || sourceLabel);
    const fp = collectionFingerprint(normalized);
  ...
  merged.sort((a,b)=>new Date(b.startedAt||0)-new Date(a.startedAt||0));
  ...
  showModal("Importação concluída", `${imported} coleta(s) importadas. ${skipped} já estavam no histórico.`, false);
}

function collectionFingerprint(c){
  return [
    c.id || "",
    c.name || "",
    c.startedAt || "",
...
async function handleBackupFile(evt){
  ...
  try{
    ...
    else if(Array.isArray(parsed.collections)) collections = parsed.collections;
    else if(looksLikeCollection(parsed)) collections = [parsed];

    collections = collections.filter(looksLikeCollection);

    if(!collections.length){
      showModal("Backup inválido","Não encontrei coletas válidas nesse arquivo JSON.",false);
      return;
    }

    importLocalCollections(collections, file.name || "backup_json");
  }catch(err){
    console.error("Erro ao importar backup", err);
    showModal("Erro ao importar","Não consegui ler esse backup JSON.",false);
  }
}
...
function cleanOldDrafts(){
  ...
  if(!found.length){
    showModal("Sem rascunhos","Não encontrei rascunhos locais para limpar.",false);
    return;
  }

  if(!confirm(`Apagar ${found.length} rascunho(s) local(is)? Faça backup antes se tiver dúvida.`)) return;
  found.forEach(k=>localStorage.removeItem(k));
  renderLocalScanResults();
  updateDiagnostics();
  showModal("Rascunhos limpos", `${found.length} chave(s) removida(s).`, false);
}
...
els.btnMenu.onclick=openDrawer;els.btnCloseDrawer.onclick=closeDrawer;els.drawer.onclick=e=>{if(e.target===els.drawer)closeDrawer()};els.btnLocate.onclick=locate;els.btnPause.onclick=togglePause;els.btnCloseModal.onclick=hideModal;els.btnMain.addEventListener("pointerdown",startHold);els.btnMain.addEventListener("pointerup",cancelHold);els.btnMain.addEventListener("pointerleave",cancelHold);els.btnMain.addEventListener("pointercancel",cancelHold);els.btnScanLocal.onclick=renderLocalScanResults;
els.btnImportBackup.onclick=openBackupImport;
els.backupFileInput.onchange=handleBackupFile;
els.btnCleanDrafts.onclick=cleanOldDrafts;
els.btnBackupAll.onclick=exportBackupAll;
els.btnClearAll.onclick=()=>{if(confirm("Apagar todo histórico?")){localStorage.removeItem(STORAGE_KEY);renderHistory()}};

initMap();initialLocateOnOpen();startGps();recoverDraft();renderHistory();updateStatus();updateMetrics();updateDiagnostics();setInterval(updateDiagnostics,2000);
