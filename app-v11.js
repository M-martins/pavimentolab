const PAVIMENTOLAB_VERSION = "v9-cache-safe-antidrift-10m";
...
function saveGpsRecord(pointBase) {
  ...
  const point = {
    ...
    ...summary,
    raw_point: true,
    used_for_segment: false,
    ignored_reason: ""
  };
  point.classe = classByIndex(point.roughness_index);

  // Sempre salva ponto bruto para diagnóstico.
  state.current.points.push(point);

  // Inicializa bloco de agregação.
  if (!state.current.pendingStartPoint) {
    state.current.pendingStartPoint = point;
    state.current.pendingLastPoint = point;
    state.current.pendingDistanceM = 0;
    state.current.pendingStats = [];
  ...
  const previous = state.current.pendingLastPoint;
  const stepDist = distanceMeters(previous, point);

  // Anti-drift: parado ou oscilação pequena do GPS não vira trecho.
  if (stepDist !== null && stepDist < 2) {
    point.ignored_reason = "gps_drift_menor_2m";
    state.current.pendingLastPoint = point;
    state.lastGpsPoint = point;
    state.motionBuffer = [];
    saveCurrentDraft();
  ...
  // Salto absurdo de GPS também não alimenta trecho.
  if (stepDist !== null && stepDist > 120) {
    point.ignored_reason = "possivel_salto_gps_maior_120m";
    state.current.pendingLastPoint = point;
    state.lastGpsPoint = point;
    state.motionBuffer = [];
    saveCurrentDraft();
  ...
  if (stepDist && stepDist >= 2 && stepDist <= 120) {
    state.current.pendingDistanceM += stepDist;
    point.used_for_segment = true;
  }

  state.current.pendingStats.push(point);
  state.current.pendingLastPoint = point;

  // Fecha trecho apenas quando acumular 10 m válidos.
  if (state.current.pendingDistanceM >= 10) {
    const startPoint = state.current.pendingStartPoint;
    const endPoint = point;
    const stats = state.current.pendingStats.filter(p => p.roughness_index !== null);

    const roughnessIndexes = stats.map(p => p.roughness_index).filter(v => typeof v === "number");
    const roughnessMeans = stats.map(p => p.roughness_mean).filter(v => typeof v === "number");
    const roughnessMaxes = stats.map(p => p.roughness_max).filter(v => typeof v === "number");
    const roughnessStds = stats.map(p => p.roughness_std).filter(v => typeof v === "number");
    const peakCounts = stats.map(p => p.peak_count).filter(v => typeof v === "number");
    const sampleCounts = stats.map(p => p.sample_count).filter(v => typeof v === "number");

    const segmentIndex = roughnessIndexes.length ? mean(roughnessIndexes) : null;
    const segmentClass = classByIndex(segmentIndex);
    const accNow = endPoint.gps_accuracy_m ?? 999;
    const accStart = startPoint.gps_accuracy_m ?? 999;

    let segment_quality = "ok";
    const flags = [];

    if (state.current.pendingDistanceM > 80) {
      flags.push("trecho_longo_gps_lento");
      segment_quality = "atencao";
    }

    if (accNow > 35 || accStart > 35) {
      flags.push("gps_baixa_precisao");
      segment_quality = "baixa_confianca";
    }

    if (!roughnessIndexes.length) {
      flags.push("sem_amostra_acelerometro");
      if (segment_quality === "ok") segment_quality = "atencao";
    }

    const segment = {
      segment_id: state.current.segments.length + 1,
      timestamp_start: startPoint.timestamp,
      timestamp_end: endPoint.timestamp,
      lat_start: startPoint.lat,
      lon_start: startPoint.lon,
      lat_end: endPoint.lat,
      lon_end: endPoint.lon,
      distance_m: state.current.pendingDistanceM,
      speed_mps: endPoint.speed_mps,
      heading: endPoint.heading,
      gps_accuracy_start_m: startPoint.gps_accuracy_m,
      gps_accuracy_end_m: endPoint.gps_accuracy_m,
      sample_count: sampleCounts.reduce((a,b) => a + b, 0),
      roughness_mean: roughnessMeans.length ? mean(roughnessMeans) : null,
      roughness_max: roughnessMaxes.length ? Math.max(...roughnessMaxes) : null,
      roughness_std: roughnessStds.length ? mean(roughnessStds) : null,
      peak_count: peakCounts.reduce((a,b) => a + b, 0),
      roughness_index: segmentIndex,
      classe: segmentClass,
      segment_quality,
      flags: flags.join(";"),
      aggregation_m: 10,
      source_points: state.current.pendingStats.length,
      anti_drift_min_step_m: 2
    };

    state.current.segments.push(segment);
    drawSegment(segment);

    if (segment.distance_m && segment.distance_m > 0 && segment.distance_m < 120) {
      state.current.totalDistanceM = (state.current.totalDistanceM || 0) + segment.distance_m;
    }

    state.current.pendingStartPoint = endPoint;
    state.current.pendingLastPoint = endPoint;
    state.current.pendingDistanceM = 0;
    state.current.pendingStats = [];
  }
...
function toCsv(collection) {
  const cols = [
    ...
    "peak_count","roughness_index","classe","raw_point","used_for_segment","ignored_reason"
  ];
  const meta = [
  ...
  const features = collection.points
    ...
    .map(r => ({
      ...
      properties: {
        ...
        classe: r.classe,
        raw_point: r.raw_point,
        used_for_segment: r.used_for_segment,
...
function toSimplifiedPointsGeoJson(collection) {
  const features = [];

  collection.segments.forEach((s, idx) => {
    const segmentId = s.segment_id || idx + 1;

    features.push({
      type: "Feature",
      geometry: { type: "Point", coordinates: [s.lon_start, s.lat_start] },
      properties: {
        point_role: "inicio_trecho",
        segment_id: segmentId,
        timestamp: s.timestamp_start,
        gps_accuracy_m: s.gps_accuracy_start_m,
    ...
    features.push({
      type: "Feature",
      geometry: { type: "Point", coordinates: [s.lon_end, s.lat_end] },
      properties: {
        point_role: "fim_trecho",
        segment_id: segmentId,
        timestamp: s.timestamp_end,
        gps_accuracy_m: s.gps_accuracy_end_m,
        distance_m: s.distance_m,
        roughness_index: s.roughness_index,
        classe: s.classe,
        segment_quality: s.segment_quality,
  ...
  return JSON.stringify({
    type: "FeatureCollection",
    name: `${collection.name}_pontos_simplificados`,
    metadata: {
      startedAt: collection.startedAt,
      endedAt: collection.endedAt,
      device: collection.device,
      ...
      aggregation_m: 10,
      anti_drift_min_step_m: 2,
      simplification: "start_end_points_per_segment",
      calibration: collection.calibration,
      visibilityEvents: collection.visibilityEvents || [],
    },
    features
  }, null, 2);
}
...
function exportCollection(collection, kind) {
  ...
  if (kind === "points") downloadText(`${collection.name}_pontos_brutos.geojson`, toPointsGeoJson(collection), "application/geo+json");
  if (kind === "simplePoints") downloadText(`${collection.name}_pontos_simplificados.geojson`, toSimplifiedPointsGeoJson(collection), "application/geo+json");
...
function showExportModal(collection) {
  ...
  els.btnExportSimplePoints.onclick = () => exportCollection(collection, "simplePoints");
...
function cleanupOldRuntimeCaches() {
  const oldKeys = [
    "pavimentolab_current",
    "pavimentolab_current_v5"
  ];
  oldKeys.forEach(k => {
    // não apaga histórico, só rascunhos antigos que causavam confusão
...
async function registerServiceWorker() {
  // v9: desativa service workers antigos para evitar mistura de versões em cache.
  if ("serviceWorker" in navigator) {
    try {
      const regs = await navigator.serviceWorker.getRegistrations();
      await Promise.all(regs.map(reg => reg.unregister()));
      console.log("Service workers antigos removidos:", regs.length);
    } catch (err) {
      console.warn("Não foi possível remover service workers antigos", err);
    }
  }
}
