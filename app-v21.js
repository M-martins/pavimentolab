...
function computeRoughness(samples){
  ...
  // Remove o viés da posição do celular definido na calibração.
  const vals = samples
  ...
  // Pico conservador para não transformar qualquer oscilação em crítico.
  const peaks=vals.filter(v=>v>4.5).length;

  // Índice suavizado.
  const index = (m * 0.45) + (std * 0.35) + (peaks * 0.05);
