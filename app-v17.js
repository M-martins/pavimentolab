const PAVIMENTOLAB_VERSION = "v17-history-finder";
...
const els = {
  ...
  historyList:document.getElementById("historyList"), localDataList:document.getElementById("localDataList"), btnScanLocal:document.getElementById("btnScanLocal"), btnClearAll:document.getElementById("btnClearAll"), btnBackupAll:document.getElementById("btnBackupAll"),
...
const STORAGE_KEY="pavimentolab_collections_v17";
const DRAFT_KEY="pavimentolab_current_v17";
const LEGACY_COLLECTION_KEYS=["pavimentolab_collections_v12","pavimentolab_collections_v13","pavimentolab_collections_v14","pavimentolab_collections_v15","pavimentolab_collections_v16"];
const LEGACY_DRAFT_KEYS=["pavimentolab_current_v12","pavimentolab_current_v13","pavimentolab_current_v14","pavimentolab_current_v15","pavimentolab_current_v16"];
...
function renderLocalScanResults(){
  ...
  els.localDataList.querySelectorAll("[data-import-local]").forEach(btn=>{
    btn.onclick=()=>{
      ...
      importLocalCollections(res.collections);
    };
  });
...
function importLocalCollections(collections){
  const existing = getCollections();
  const byId = new Map(existing.map(c=>[c.id,c]));
  let imported = 0;

  collections.forEach(c=>{
    const normalized = normalizeRecoveredCollection(c, c.recoveredFrom || "local_scan");
  ...
  const merged = Array.from(byId.values()).sort((a,b)=>new Date(b.startedAt||0)-new Date(a.startedAt||0));
  localStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
  renderHistory();
  showModal("Importação concluída", `${imported} coleta(s) importadas para o histórico atual.`, false);
}
