...
const PAVIMENTOLAB_VERSION = "v21.5-indexeddb-calibracao-modal-antiga";
...
async function calibrateSensors(){
  ...
  // Android normalmente não exige permissão explícita de movimento,
  // mas este bloco preserva compatibilidade com navegadores que exigem.
  try{
  ...
  while(Date.now() - start < CALIBRATION_MS){
    ...
    const remaining = Math.max(0, Math.ceil((CALIBRATION_MS - (Date.now() - start)) / 1000));
    els.modalText.textContent =
      `Deixe o celular parado por 10 segundos antes de iniciar. Faltam ${remaining}s. Amostras: ${samples.length}`;
  ...
  try{
    localStorage.setItem("pavimentolab_calibration_v21_5", JSON.stringify(calibration));
  }catch(err){
...
boot().catch(err=>{
  console.error(err);
  alert("Erro ao iniciar PavimentoLab v21.5: " + (err.message || err));
});
